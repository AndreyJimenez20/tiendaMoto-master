...
export default function Portafolio() {
  ...
  const fetchUser = async () => {
    ...
    try {
      const responseUser = await axios.get(`http://localhost:4001/api/usuario/${email}`);
  ...
  const handleRecibirInfo = async (nombreMoto: string) => {
    ...
    if (token) {
      ...
      if (confirmacion) {
        await axios.post('http://localhost:4001/api/cliente', infoCliente);
      }
    } else {
  ...
  const handleBusqueda = async () => {
    ...
    if (busqueda.trim() === "") {
      ...
      try {
        const response = await axios.get("http://localhost:4001/api/moto");
      ...
      try {
        const response = await axios.get(`http://localhost:4001/api/moto/busca/${busqueda}`);
        setMotos(response.data);
      } catch (error) {
