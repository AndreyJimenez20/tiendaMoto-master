...
export default function FuncionesAdmin() {
    ...
    useEffect(() => {
        const fetchMotos = async () => {
            try {
                const response = await axios.get<IMoto[]>("http://localhost:4001/api/moto");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isEditing && editId) {
                // Actualizar moto
                const response = await axios.put(`http://localhost:4001/api/moto/${editId}`, dataForm);
                setMotos((prev) => prev.map((moto) => (moto._id === editId ? response.data : moto)));
                setIsEditing(false);
                setEditId(null);
            } else {
                // Crear nueva moto
                const response = await axios.post("http://localhost:4001/api/moto", dataForm);
    ...
    const handleDelete = async (id: string) => {
        try {
            await axios.delete(`http://localhost:4001/api/moto/${id}`);
